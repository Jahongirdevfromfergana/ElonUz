...
import uz.fergana.elonuz.databinding.GridCategoryItemLayoutBinding

class GridCategoryAdapter(val items:List<CategoryModel>, val fullItems:List<CategoryModel>):RecyclerView.Adapter<GridCategoryAdapter.Vh>() {
    inner class Vh(val binding: GridCategoryItemLayoutBinding):RecyclerView.ViewHolder(binding.root)

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): Vh {
        return Vh(GridCategoryItemLayoutBinding.inflate(LayoutInflater.from(parent.context),parent,false))
    }
